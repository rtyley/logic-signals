...
//  }
  ...
  def validate[T, C](channelSignals: ChannelSignals[T, C], criterion: Criterion[C])(using time: Time[T]): Seq[TransitionPair[T, C]] = {
    (for {
      (startChannel, startSignal) <- channelSignals.data if criterion.start.appliesTo(startChannel)
      (endChannel, endSignal) <- channelSignals.data if criterion.end.appliesTo(endChannel)
      startEvent <- startSignal.events() if criterion.start.triggeredByValueOf(startEvent)
      endSignalFromStartingEventOnwards = endSignal.subInterval(atOrAbove(startEvent.time))
      subsequentEndEvent <- endSignalFromStartingEventOnwards.events().find(criterion.end.triggeredByValueOf)
