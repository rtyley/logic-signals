...
object BoundedInterval {
  ...
  extension [A](ns: BoundedInterval[A] | Above[A])
    def lowerValueBound: ValueBound[A] =
      ns.lowerBound.asInstanceOf[ValueBound[A]] // Bounded, Point & Above all have lowerBound of type ValueBound

  extension [A](ns: BoundedInterval[A] | Below[A])
    def upperValueBound: ValueBound[A] =
      ns.upperBound.asInstanceOf[ValueBound[A]] // Bounded, Point & Below all have upperBound of type ValueBound
}
